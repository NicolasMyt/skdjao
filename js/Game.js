class Game {
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        var lifePlayers = allPlayers[plr].life
        if (lifePlayers <= 0){
          this.gameOverObstacle()
        } 
  ...
  handlePlayerControls() {
    player.update();
    if (keyIsDown(UP_ARROW) && player.fuel > 0 && player.life > 0) {
      // atualize os valores aqui
      player.fuel -= 0.5
      carSound.setVolume(1,0.5)
    ...
    if (!keyIsDown(UP_ARROW) && player.SpeedY > 0 || player.SpeedY > 0 && player.fuel <= 0 || player.SpeedY > 0 && player.life <= 0){
      player.SpeedY -= 0.5
      carSound.setVolume(0,1)
    }

    if (keyIsDown(LEFT_ARROW) && player.positionX > width / 3 - 50 && player.fuel > 0 && player.life > 0) {
      player.positionX -= 5;
    }

    if (keyIsDown(RIGHT_ARROW) && player.positionX < width / 2 + 300 && player.fuel > 0 && player.life > 0) {
      player.positionX += 5;
    }
  }
